...
lending_stack <- 
  ...
  add_candidates(lendign_rf_tune) %>%
...
lending_final_stack(lending_training)


  
preds <- lending_final_test %>%
  collect_predictions()

preds

conf_mat(preds, Class, .pred_good) 

ggplot(aes(x = .pred_good, fill = Class)) + geom_density()
